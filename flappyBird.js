...
class Bird {
  ...
  update(speed) {
    ...
    if (gameStarted) {
      ...
      if (this.birdRotateAngle + angleFactor < 90)
        this.birdRotateAngle += angleFactor;
...
class Pipe {
  ...
  static create() {
    ...
    new Pipe(
      new Vec(backgroundWidth, lowerPipeYPosition),
      pipeImg,
      backgroundHeight - lowerPipeYPosition
    );
    let upperPipeYPosition = 0 - (Pipe.pipesVerticalSpacing - randomPipeFactor);
    new Pipe(
      new Vec(backgroundWidth, upperPipeYPosition),
      upperPipeImg,
      upperPipeYPosition + backgroundHeight / 2
    );
  }
  ...
  static update() {
    ...
    //adding pipes when the first ones reach the middle of the screen
      ...
      Pipe.pipesInState[0].position.x <= backgroundWidth / 2 - Pipe.pipeWidth &&
      !Pipe.werePipesCreatedRecently
    ) {
...
class Ground {
  ...
  static update() {
    ...
    if (
      Ground.groundPiecesInState[0].position.x <=
      0 - Ground.groundPieceWidth
    ) {
      Ground.groundPiecesInState.shift();
      Ground.wasGroundCreatedRecently = false;
    }
    if (
      Ground.groundPiecesInState[0].position.x <= 0 &&
      !Ground.wasGroundCreatedRecently
    ) {
      let newGroundHorizontalPosition =
        Ground.groundPiecesInState[Ground.groundPiecesInState.length - 1]
          .position.x + Ground.groundPieceWidth;
...
function drawBirdAtAnAngle() {
  ...
  if (gameStarted)
    ctx.drawImage(
      birdAnimation[parseInt(frame / 5)],
      -birdSize.x / 2,
      -birdSize.y / 2,
      birdSize.x,
      birdSize.y
...
function endOfGameAnimation() {
  ...
  if (bird.position.y + Bird.size.y < backgroundHeight + 20)
    requestAnimationFrame(endOfGameAnimation);
  else {
...
function getCursorPosition(canvas, event) {
  if (gameOver && restartEnabled) {
    ...
    if (event.type === "mousedown") {
      ...
      if (touchX >= 125 && touchX <= 338.5 && touchY >= 280 && touchY <= 353.5)
        restart();
    }
  }
}
...
function stateUpdate() {
  if (gameStarted) {
    ...
    if (Math.abs(bird.speed.y) < startingScreenSpeedLimit.y)
      bird.update(startingScreenGravity);
    else {
...
function collisionDetection() {
  ...
  if (doesCollideWithPipes) {
    for (let pipe of Pipe.pipesInState) {
      if (
        bird.position.x + Bird.size.x >= pipe.position.x + 35 &&
        bird.position.x <= pipe.position.x + pipe.size.x
      ) {
        let pipeEnd;
